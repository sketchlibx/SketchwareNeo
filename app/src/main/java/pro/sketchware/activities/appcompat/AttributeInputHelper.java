...
final class AttributeInputHelper {
    ...
    static void wire(Activity activity, String scId, EditText inputAttr, TextInputLayout inputValueLayout, EditText inputValue) {
    ...
    private static void showResourcePicker(Activity activity, String scId, EditText inputValue) {
        ...
        try {
            names.addAll(jC.d(scId).m());
            names.addAll(new mod.bobur.VectorDrawableLoader().getVectorDrawables(scId));
        } catch (Exception ignored) {
        }

        if (names.isEmpty()) {
            pro.sketchware.utility.SketchwareUtil.toast("No drawable resources found for this project.");
            return;
        }

        String[] namesArray = names.toArray(new String[0]);
        new MaterialAlertDialogBuilder(activity)
                .setTitle("Select drawable")
                .setItems(namesArray, (dialog, which) -> inputValue.setText("@drawable/" + namesArray[which]))
    ...
    private static void setupChoiceInput(Activity activity, TextInputLayout inputValueLayout, EditText inputValue, String[] options) {
        ...
        makeTapToPick(inputValueLayout, inputValue, R.drawable.ic_mtrl_arrow_down, openPicker);
    }
